...
export default {
    ...
    methods: {
        enableSend() {
            if(this.config.qr_api_url != '' && this.config.qr_api_apiKey != '') {
        ...
        async sendQrChat() {
            ...
            this.convertFileToBase64(this.wsFile)
                .then(file_encode64 => {
                    ...
                    return this.$http
                        .post(`${this.config.qr_api_url}\\api\\message\\send\\pdf`, this.form, {
                            headers: {
                                "Authorization" : `Bearer ${this.config.qr_api_apiKey}`
                            }
                        })
                        .then(response => {
